...
def argparser():
    ...
    parser.add_argument('-test_path', default='../../input/2-stage-dcm-test/', type=str, nargs='?', help='directory with test')
    parser.add_argument('-rle_path', default='../../input/stage_2_train.csv', type=str, nargs='?', help='path for rle csv file')
    parser.add_argument('-out_path', default='../../input/dataset', type=str, nargs='?', help='path for saving dataset')
...
def save_train_file(f, encode_df, out_path, img_size):
    ...
    encode = resize(encode,(img_size,img_size)) # 此处的resize会把数字归一化到0-1，所以后面要*255
    img = resize(img,(img_size,img_size))

    cv2.imwrite('{}/train/{}.png'.format(out_path, name), img * 255) # image和mask都是0-255
...
def main():
    ...
    n_train = args.n_train # 训练数据有多少个，没有设置的话，为glob出来的个数
    n_threads = args.n_threads
    
    rle = None
    if os.path.exists(args.rle_path):
